...
const endIdx = content.indexOf('document.getElementById("recapSlide").innerHTML = html;\n  }', startIdx) + 59;
...
let existing = fs.readFileSync('js/app.js', 'utf8');
let fixed = existing.replace(/^[ \t]*\/\* FIXX \*\/[\s\S]*?\}\s*\}$/m, ''); // remove the broken function completely if it exists.
if (fixed === existing) {
   let fixed = before + newRender + after;
   fs.writeFileSync('js/app.js', fixed, 'utf8');
} else {
   // The broken function is already removed or changed. Let's just restore original state from git and patch it again!
}
