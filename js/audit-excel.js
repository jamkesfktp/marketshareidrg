(function marketShareAuditExcel(global) {
  ...
  function formatHospitalList(list, formatItemFn, maxChars) {
    if (!list || list.length === 0) return "Tidak ada";
    const limit = maxChars || 30000;
    const items = [];
    let currentLength = 0;
    for (let i = 0; i < list.length; i++) {
      const text = formatItemFn ? formatItemFn(list[i]) : list[i].name;
      const separator = items.length > 0 ? "; " : "";
      const remaining = list.length - i;
      const suffix = remaining > 1 ? `; +${remaining} RS lainnya` : "";
      if (currentLength + separator.length + text.length + suffix.length > limit) {
        items.push(`+${remaining} RS lainnya`);
        break;
      }
      items.push(text);
      currentLength += separator.length + text.length;
    }
    return items.join("; ");
  }
  ...
  function buildWorkbook(context) {
    ...
    const guideRows = [
      ...
      ["Hijau", "Perhitungan tambahan kasus dan pendapatan INACBG."],
    ...
    data.services.forEach((service) => {
      ...
      competencyRows.push([
        ...
        targetCompetency,
        levelNames[targetCompetency] || "Tidak Kompeten",
        groups[1].length,
        formatHospitalList(groups[1]),
        groups[2].length,
        formatHospitalList(groups[2]),
        groups[3].length,
        formatHospitalList(groups[3]),
        groups[4].length,
        formatHospitalList(groups[4]),
        capable.length,
        capable.length ? formatHospitalList(capable, (hospital) => `${hospital.name} (${levelNames[getCompetency(hospital, service)]})`) : "Tidak ada kompetitor",
      ]);
    });
    ...
    data.services.forEach((service, serviceIndex) => {
      ...
      severityRanks.forEach((rank) => {
        ...
        serviceRows.push([
          ...
          formula(`${compSheet}!N${compSourceRow}`, capableCompetitors.length ? formatHospitalList(capableCompetitors, (hospital) => `${hospital.name} (${levelNames[getCompetency(hospital, service)]})`) : "Tidak ada kompetitor", undefined, "s"),
        ]);
    ...
    data.services.forEach((service) => {
      ...
      state.serviceScenarios[service] = Array.from({ length: 6 }, (_, index) => {
        const scenario = {};
        const defaultLowLevels = [1, 2, 3, 4, 5, 10];
        rules.tambah.forEach((rank) => {
          if (rank === 1 || rank === 2) {
            scenario[`tambah_${rank}`] = defaultLowLevels[index];
          } else {
            ...
            scenario[`tambah_${rank}`] = Math.min(100, Math.max(0, base + (index * 2)));
          }
        });
    ...
    data.services.forEach((service) => {
      ...
      scenarios.forEach((scenario, index) => {
        ...
        parameterRows.push([
          service,
          formatService(service),
          levelNames[competency] || "Tidak Kompeten",
          index + 1,
    ...
    function buildServiceSheet(service, serviceIndex) {
      ...
      rows.push(["Kompetensi", competency, levelNames[competency] || "Tidak Kompeten", "", "Jumlah RS", data.hospitals.length, "", "", "Eksisting", formula("IFERROR(B6/F6,0)", regionalMetric[CASES] ? targetMetric[CASES] / regionalMetric[CASES] : 0, percentFormat), "Setara/lebih tinggi", formula(`${quoteSheet(serviceDataName)}!Q${firstSourceRow}`, capableCompetitors.length, numberFormat)]);
      ...
      rows.push(["Pendapatan INACBG", formula(`SUM(${quoteSheet(serviceDataName)}!J${sourceRows[1]},${quoteSheet(serviceDataName)}!J${sourceRows[2]},${quoteSheet(serviceDataName)}!J${sourceRows[3]},${quoteSheet(serviceDataName)}!J${sourceRows[4]})`, Number(targetMetric[IDRG]) || 0, moneyFormat), "", "", "Pendapatan INACBG", formula(`SUM(${quoteSheet(serviceDataName)}!M${sourceRows[1]},${quoteSheet(serviceDataName)}!M${sourceRows[2]},${quoteSheet(serviceDataName)}!M${sourceRows[3]},${quoteSheet(serviceDataName)}!M${sourceRows[4]})`, Number(regionalMetric[IDRG]) || 0, moneyFormat)]);
      rows.push([]);
      ...
      rows.push([]);
      rows.push([competency ? "ASUMSI SKENARIO - edit parameter pada sheet 03_Parameter" : "LAYANAN TIDAK DAPAT DISIMULASIKAN: kompetensi target Tidak Kompeten"]);
      ...
      for (let scenarioIndex = 1; scenarioIndex <= 6; scenarioIndex += 1) {
        ...
        reduceLevels.forEach((rank) => {
          ...
          const existingIdrg = Number(severityMetric(target.services?.[service], rank)[IDRG]) || 0;
          reducedCases += existingCases * pct;
          reducedIna += existingIdrg * pct;
          ...
          row[colIndex[`redIna${rank}`]] = formula(`${quoteSheet(serviceDataName)}!J${sourceRow}*${pctCol}${excelRow}`, existingIdrg * pct, moneyFormat);
        ...
        const sisaIdrg = (Number(targetMetric[IDRG]) || 0) - reducedIna;
        const projectedRevenue = sisaIdrg + addedIdrg;
        const deltaCasesCol = excelColumn(colIndex.deltaCase);
        const deltaRevenueCol = excelColumn(colIndex.deltaRevenue);
        const projectedRevenueCol = excelColumn(colIndex.projectedRevenue);
        ...
        row[colIndex.projectedRevenue] = formula(`($B$8-(${reduceInaCells.length ? reduceInaCells.join("+") : "0"}))+(${addIdrgCells.length ? addIdrgCells.join("+") : "0"})`, projectedRevenue, moneyFormat);
        row[colIndex.deltaRevenuePct] = formula(`IF($B$7=0,0,(${projectedRevenueCol}${excelRow}-$B$7)/$B$7)`, targetMetric[INA] ? (projectedRevenue - targetMetric[INA]) / targetMetric[INA] : 0, percentFormat);
        rows.push(row);
      ...
      const safeScenarios = scenarioList.filter((s) => (Number(targetMetric[CASES]) || 0) === 0 || s.projectedCases <= (Number(targetMetric[CASES]) || 0));
      const closestVolumeScenario = safeScenarios.length > 0 
        ? safeScenarios.reduce((best, current) => (!best || current.deltaRevenue > best.deltaRevenue || (current.deltaRevenue === best.deltaRevenue && current.addedCases > best.addedCases) ? current : best), null)
        : scenarioList[0];
    ...
    serviceSheetMeta.forEach((meta, serviceIndex) => {
      ...
      const formatCellArr = (minV, maxV, isRp) => {
        ...
        const fMin = isRp ? (Math.abs(minV)/1000000000).toFixed(2) + " M" : Math.abs(minV).toFixed(0);
        const fMax = isRp ? (Math.abs(maxV)/1000000000).toFixed(2) + " M" : Math.abs(maxV).toFixed(0);
      ...
      const rowData = [
        ...
        meta.service,
        meta.displayName,
        levelNames[meta.competency] || "Tidak Kompeten"
      ];
      ...
      rowData.push((existingIna/1000000000).toFixed(2) + " M");
