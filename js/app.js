...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderTariffScatterWorkspace() {
    ...
    if (!source?.scatterServices) { root.innerHTML = '<div class="scatter-empty">Data tarif aktual scatter belum tersedia. Muat ulang halaman tanpa cache (Ctrl+F5) untuk mengambil bundel data terbaru.</div>'; return; }
    ...
    [["scatterDataSource","dataSource"],["scatterService","service"],["scatterPtd","ptd"],["scatterRegion","region"],["scatterHospitalClass","hospitalClass"],["scatterRawatClass","rawatClass"],["scatterOwnership","ownership"]].forEach(([id,key])=>root.querySelector(`#${id}`)?.addEventListener("change",(event)=>{ui[key]=event.target.value;rerender();}));
    ...
    root.querySelector("#scatterSave")?.addEventListener("click",()=>{const output=document.createElement("canvas");output.width=1800;output.height=1320;const ctx=output.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,output.width,output.height);ctx.fillStyle="#07575b";ctx.font="bold 36px Arial";ctx.fillText("Analisis Selisih Tarif iDRG terhadap INA-CBG",45,55);ctx.font="18px Arial";ctx.fillStyle="#52615f";ctx.fillText(`${scatterScenarioLabel} · ${ui.service === "ALL" ? "ALL Layanan" : ui.service} · PTD ${ui.ptd === "1" ? "Rawat Inap" : ui.ptd === "2" ? "Rawat Jalan" : "ALL"} · ${ui.region} · ${ui.hospitalClass} · ${ui.rawatClass} · ${ui.ownership}`,45,88);ctx.drawImage(chartCanvas,45,115,1710,850);ctx.fillStyle="#07575b";ctx.font="bold 23px Arial";ctx.fillText("Deskripsi outlier utama · rata-rata aktual per kasus",45,1010);ctx.font="16px Arial";topPoints.forEach((point,index)=>{ctx.fillStyle=point.realY>=0?"#b53b2e":"#2879ad";ctx.fillText(`${index+1}. ${point.code} · ${point.realY>=0?"▲ ":"▼ "}${Math.abs(point.realY).toFixed(1)}% · ${formatNumber(point.cases)} kasus`,55,1045+index*24);ctx.fillStyle="#34464a";ctx.fillText(point.description.slice(0,120),390,1045+index*24);});const link=document.createElement("a");link.download=`Scatter_Tarif_iDRG_${useAf19 ? "AF19" : useAf10 ? "AF10" : "Awal"}_${new Date().toISOString().slice(0,10)}.png`;link.href=output.toDataURL("image/png");link.click();});
  }
