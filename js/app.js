        ...
        <table class="dynamic-scenario-table" style="width:100%;border-collapse:collapse;border:1px solid #1e293b;text-align:center;font-size:11px;">
          ...
          <tbody>${scenarioResults.map((result) => {
            ...
            return `<tr style="background:${result.scenarioIndex === mostLogicalIdx ? "#ecfeff" : "#fff"};">
              ...
              ${result.scenarioIndex === 0 ? `<td rowspan="${scenarioResults.length}" style="border:1px solid #1e293b;padding:7px;background:#f8fafc;min-width:175px;"><div style="display:flex;justify-content:center;gap:6px;font-size:9px;">${levelData.map((item) => `<span style="font-weight:${rules.tambah.includes(item.level) ? '900' : '400'};color:${rules.tambah.includes(item.level) ? '#0f172a' : '#64748b'};">${shortLevelNames[item.level]}: ${formatNumber(item.targetCases)}</span>`).join('')}</div><div style="font-size:16px;font-weight:900;color:#0f172a;margin:3px 0;">${formatNumber(baselineCases)} kasus</div><div style="font-size:10px;font-weight:800;color:#c2410c;">INA ${formatTableMoney(baselineIna)}</div><div style="font-size:10px;font-weight:800;color:#0369a1;">iDRG ${formatTableMoney(baselineIdrg)}</div></td>` : ""}
      ...
      <div style="overflow-x:auto;width:100%;"><table style="width:100%;border-collapse:collapse;border:1px solid #1e293b;text-align:center;font-size:10px;">
        <thead><tr>
          ...
          return `<tr style="background:${result.scenarioIndex === mostLogicalIdx ? '#ecfeff' : '#fff'};"><td style="${cell}font-weight:900;white-space:nowrap;">Skenario ${result.scenarioIndex + 1}${result.scenarioIndex === mostLogicalIdx ? ' <span style="font-size: 8px; color: #ffffff; background: #ea580c; padding: 1px 4px; border-radius: 3px;">⚡ Paling Logis</span>' : ''}<div style="font-size:8px;color:#64748b;">${result.definition.name} · +${result.definition.add}% natural</div></td>
            ${result.scenarioIndex === 0 ? `<td rowspan="${results.length}" style="${cell}background:#f8fafc;min-width:145px;"><div style="display:flex;justify-content:center;gap:5px;font-size:8px;">${levelData.map((item) => `<span style="font-weight:${rules.tambah.includes(item.level) ? '900' : '400'};color:${rules.tambah.includes(item.level) ? '#0f172a' : '#64748b'};">${shortLevelNames[item.level]}: ${formatNumber(item.targetCases)}</span>`).join('')}</div><div style="font-size:16px; font-weight:900; color:#0f172a; margin-bottom:4px;">${formatNumber(baselineCases)} kasus</div><div style="font-size:11px; font-weight:800; color:#c2410c;">INA ${formatTableMoney(baselineIna)}</div><div style="font-size:11px; font-weight:800; color:#0369a1;">iDRG ${formatTableMoney(baselineIdrg)}</div></td>` : ''}
