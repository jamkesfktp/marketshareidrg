...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderIdrgMapSlide() {
    ...
    const inaTariff = (code) => {
      ...
      if (ui.hospitalClass === "WITH_SPECIAL") {
      ...
      <div style="height:100%;display:flex;flex-direction:column;background:#fff;overflow:hidden;">
        <div style="padding:12px 20px;border-top:5px solid #111;background:#fff;display:flex;align-items:center;justify-content:space-between;gap:15px;">
          ...
          <div style="display:flex;gap:7px;align-items:end;flex-wrap:wrap;justify-content:flex-end;">
            ...
            <label style="font-size:9px;font-weight:900;color:#475569;">CAKUPAN KELAS RS<br><select id="idrgMapHospitalClass" style="padding:6px;border:1px solid #94a3b8;border-radius:6px;font-weight:750;"><option value="WITHOUT_SPECIAL" ${ui.hospitalClass === "WITHOUT_SPECIAL" ? "selected" : ""}>Tanpa RS Khusus (A–D)</option><option value="WITH_SPECIAL" ${ui.hospitalClass === "WITH_SPECIAL" ? "selected" : ""}>Dengan RS Khusus</option></select></label>
    ...
    root.querySelector("#idrgMapExcel")?.addEventListener("click", (event) => {
      ...
      try {
        ...
        rows.forEach((row, index) => {
          ...
          exportRows.push([ui.service, sevLabels[row.severity] || "Tanpa Severity", row.ina, row.inaDescription, regionLabel, ui.hospitalClass === "WITH_SPECIAL" ? "Dengan RS Khusus" : "Tanpa RS Khusus (A–D)", ui.rawatClass, ui.ownership, inaValue, row.idrg, source.idrgTariffs?.[row.idrg]?.description || row.idrgDescription, idrgValue, row.cases,
