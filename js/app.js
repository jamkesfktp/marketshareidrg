...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderTariffScatterWorkspace() {
    ...
    window.tariffScatterUi = { service: "ALL", region: "ALL", hospitalClass: "ALL", rawatClass: "ALL", ownership: "ALL", ptd: "ALL", threshold: 6, ...(window.tariffScatterUi || {}) };
    ...
    selectedServices.forEach((service) => (source.scatterServices[service] || []).forEach((segment) => {
      ...
      const cases = Number(segment[6]) || 0, inaTotal = Number(segment[7]) || 0, idrgTotal = Number(segment[scenarioMetricIndex]) || 0;
      if (!cases || !inaTotal || !idrgTotal) return;
