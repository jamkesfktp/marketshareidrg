...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderTariffScatterWorkspace() {
    ...
    const matchesSegment = (segment) => (ui.ownership === "ALL" || segment[0] === ui.ownership) && (ui.hospitalClass === "ALL" || segment[1] === ui.hospitalClass) && (ui.rawatClass === "ALL" || segment[2] === ui.rawatClass) && (ui.region === "ALL" || segment[3] === ui.region);
    const relationCases = (row) => Array.isArray(row.segments) ? row.segments.reduce((sum, segment) => sum + (matchesSegment(segment) ? Number(segment[4]) || 0 : 0), 0) : Number(row.cases) || 0;
    const inaTariff = (code) => {
      const item = source.inaTariffs?.[code] || {};
      const classRates = ui.ownership === "ALL" ? item.rates || {} : item.ownershipRates || {};
      const values = [];
      (ui.hospitalClass === "ALL" ? Object.keys(classRates) : [ui.hospitalClass]).forEach((hospitalClass) => {
        const rawatRates = classRates[hospitalClass] || {};
        (ui.rawatClass === "ALL" ? Object.keys(rawatRates) : [ui.rawatClass]).forEach((rawatClass) => {
          const rates = ui.ownership === "ALL" ? rawatRates[rawatClass] || {} : rawatRates[rawatClass]?.[ui.ownership] || {};
          if (hospitalClass === "RS KHUSUS") { if (Number(rates.ALL) > 0) values.push(Number(rates.ALL)); }
          else if (ui.region === "ALL") Object.entries(rates).forEach(([key, value]) => { if (/^R[1-5]$/.test(key) && Number(value) > 0) values.push(Number(value)); });
    ...
    selectedServices.forEach((service) => (source.services[service] || []).forEach((row) => {
      const cases = relationCases(row); const ina = inaTariff(row.ina); const idrg = Number(source.idrgTariffs?.[row.idrg]?.tariff) || 0;
      if (!cases || !ina || !idrg) return;
      const item = aggregate.get(row.idrg) || { code: row.idrg, description: source.idrgTariffs?.[row.idrg]?.description || row.idrgDescription, cases: 0, inaWeighted: 0, idrgWeighted: 0, services: new Set() };
      item.cases += cases; item.inaWeighted += ina * cases; item.idrgWeighted += idrg * cases; item.services.add(service); aggregate.set(row.idrg, item);
    }));
    const points = [...aggregate.values()].map((item) => { const ina = item.inaWeighted / item.cases, idrg = item.idrgWeighted / item.cases, delta = (idrg - ina) / ina * 100; return { x: item.cases, y: delta, code: item.code, description: item.description, ina, idrg, cases: item.cases, services: [...item.services].join(", "), segment: delta < 0 ? "negative" : delta <= ui.threshold ? "within" : "high" }; }).filter((item) => Number.isFinite(item.y)).sort((a,b) => Math.abs(b.y) - Math.abs(a.y));
    ...
    root.innerHTML = `<div class="scatter-shell"><header><div><span>ANALISIS TARIF NASIONAL</span><h1>Scatter Plot Selisih Tarif iDRG terhadap INA-CBG</h1><p>Volume kasus, persentase selisih tarif, segmentasi threshold, dan identifikasi outlier DRG.</p></div><div><button id="scatterSave" type="button">⬇ Simpan Gambar + Deskripsi</button><button id="scatterClose" type="button" aria-label="Tutup">×</button></div></header><section class="scatter-controls"><label>Kelompok layanan<select id="scatterService">${selectOptions(services,ui.service,"ALL Layanan")}</select></label><label>Regional<select id="scatterRegion">${selectOptions(["R1","R2","R3","R4","R5"],ui.region,"ALL Regional")}</select></label><label>Kelas RS<select id="scatterHospitalClass">${selectOptions(["A","B","C","D","RS KHUSUS"],ui.hospitalClass,"ALL Kelas RS")}</select></label><label>Kelas rawat<select id="scatterRawatClass">${selectOptions(["KELAS0","KELAS1","KELAS2","KELAS3"],ui.rawatClass,"ALL Kelas Rawat")}</select></label><label>Kepemilikan<select id="scatterOwnership">${selectOptions(["PEMERINTAH","SWASTA"],ui.ownership,"ALL Kepemilikan")}</select></label><label>Threshold positif<input id="scatterThreshold" type="number" min="0" max="100" step="1" value="${ui.threshold}"><span>%</span></label></section><main><section class="scatter-chart-panel"><div class="scatter-legend"><span><i class="high"></i>Di atas threshold</span><span><i class="within"></i>Di dalam threshold</span><span><i class="negative"></i>Selisih negatif</span><b>${formatNumber(points.length)} DRG</b></div><div class="scatter-canvas-wrap"><canvas id="tariffScatterChart" aria-label="Scatter plot volume kasus dan selisih tarif"></canvas></div></section><aside class="scatter-detail"><div id="scatterSelectedDetail"><span>PILIH TITIK</span><h2>Klik titik scatter plot</h2><p>Detail kode, deskripsi, volume, dan perbandingan tarif akan ditampilkan di sini.</p></div><h3>Outlier utama</h3><div class="scatter-outlier-list">${points.slice(0,12).map((point,index) => `<button type="button" data-scatter-code="${escapeHtml(point.code)}"><b>${index+1}</b><span><strong>${escapeHtml(point.code)}</strong><small>${escapeHtml(point.description)}</small></span><em>${point.y >= 0 ? "+" : ""}${point.y.toFixed(1)}%</em></button>`).join("") || '<p>Tidak ada data pada filter ini.</p>'}</div></aside></main></div>`;
    ...
    function showDetail(point){root.querySelector("#scatterSelectedDetail").innerHTML=`<span>DETAIL TITIK SCATTER</span><h2>${escapeHtml(point.code)}</h2><p>${escapeHtml(point.description)}</p><dl><div><dt>Volume</dt><dd>${formatNumber(point.cases)} kasus</dd></div><div><dt>Rata-rata INA-CBG</dt><dd>${moneyFormat(point.ina)}</dd></div><div><dt>Tarif iDRG</dt><dd>${moneyFormat(point.idrg)}</dd></div><div><dt>Selisih</dt><dd class="${point.y>=0?"positive":"negative"}">${point.y>=0?"+":""}${point.y.toFixed(2)}%</dd></div></dl><small>Layanan: ${escapeHtml(point.services)}</small>`;}
    ...
    [["scatterService","service"],["scatterRegion","region"],["scatterHospitalClass","hospitalClass"],["scatterRawatClass","rawatClass"],["scatterOwnership","ownership"]].forEach(([id,key])=>root.querySelector(`#${id}`)?.addEventListener("change",(event)=>{ui[key]=event.target.value;rerender();}));
    ...
    root.querySelector("#scatterSave")?.addEventListener("click",()=>{const output=document.createElement("canvas");output.width=1800;output.height=1250;const ctx=output.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,output.width,output.height);ctx.fillStyle="#07575b";ctx.font="bold 36px Arial";ctx.fillText("Analisis Selisih Tarif iDRG terhadap INA-CBG",45,55);ctx.font="18px Arial";ctx.fillStyle="#52615f";ctx.fillText(`${ui.service === "ALL" ? "ALL Layanan" : ui.service} · ${ui.region} · ${ui.hospitalClass} · ${ui.rawatClass} · ${ui.ownership}`,45,88);ctx.drawImage(chartCanvas,45,115,1710,850);ctx.fillStyle="#07575b";ctx.font="bold 23px Arial";ctx.fillText("Deskripsi outlier utama",45,1010);ctx.font="16px Arial";points.slice(0,8).forEach((point,index)=>{ctx.fillStyle=point.y>=0?"#b53b2e":"#2879ad";ctx.fillText(`${index+1}. ${point.code} · ${point.y>=0?"+":""}${point.y.toFixed(1)}% · ${formatNumber(point.cases)} kasus`,55,1045+index*24);ctx.fillStyle="#34464a";ctx.fillText(point.description.slice(0,120),390,1045+index*24);});const link=document.createElement("a");link.download=`Scatter_Tarif_iDRG_${new Date().toISOString().slice(0,10)}.png`;link.href=output.toDataURL("image/png");link.click();});
  }
  ...
  function renderIdrgMapSlide() {
    ...
    window.idrgMapUi = { service: Object.keys(source.services)[0], region: "ALL", hospitalClass: "ALL", rawatClass: "ALL", ownership: "ALL", severity: "ALL", search: "", page: 0, view: "summary", ...(window.idrgMapUi || {}) };
    ...
    root.querySelector("#idrgMapExcel")?.addEventListener("click", (event) => {
      ...
      try {
        ...
        const guideRows = [
          ["KERTAS KERJA PETA iDRG"], [], ["Parameter", "Nilai"], ["Kelompok layanan", ui.service], ["Tarif INA-CBG", regionLabel], ["Kelas RS", ui.hospitalClass], ["Kelas Rawat", ui.rawatClass], ["Kepemilikan", ui.ownership],
        ...
        const exportRows = [["Kelompok Layanan", "Severity", "Kode INA-CBG", "Deskripsi INA-CBG", "Pilihan Regional", "Kelas RS", "Kelas Rawat", "Kepemilikan", "Tarif INA-CBG (Rp)", "Kode iDRG", "Deskripsi iDRG", "Tarif iDRG +AF (Rp)", "Jumlah Kasus", "Selisih Tarif (Rp)", "% Selisih Tarif"]];
        rows.forEach((row, index) => {
          const excelRow = index + 2; const inaValue = inaTariff(row.ina); const idrgValue = source.idrgTariffs?.[row.idrg]?.tariff || 0;
          exportRows.push([ui.service, sevLabels[row.severity] || "Tanpa Severity", row.ina, row.inaDescription, regionLabel, ui.hospitalClass, ui.rawatClass, ui.ownership, inaValue, row.idrg, source.idrgTariffs?.[row.idrg]?.description || row.idrgDescription, idrgValue, row.cases,
