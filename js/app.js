...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderTariffScatterWorkspace() {
    ...
    root.querySelector("#scatterSave")?.addEventListener("click",()=>{const output=document.createElement("canvas");output.width=1800;output.height=1250;const ctx=output.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,output.width,output.height);ctx.fillStyle="#07575b";ctx.font="bold 36px Arial";ctx.fillText("Analisis Selisih Tarif iDRG terhadap INA-CBG",45,55);ctx.font="18px Arial";ctx.fillStyle="#52615f";ctx.fillText(`${scatterScenarioLabel} · ${ui.service === "ALL" ? "ALL Layanan" : ui.service} · ${ui.region} · ${ui.hospitalClass} · ${ui.rawatClass} · ${ui.ownership}`,45,88);ctx.drawImage(chartCanvas,45,115,1710,850);ctx.fillStyle="#07575b";ctx.font="bold 23px Arial";ctx.fillText("Deskripsi outlier utama · rata-rata aktual per kasus",45,1010);ctx.font="16px Arial";points.slice(0,8).forEach((point,index)=>{ctx.fillStyle=point.y>=0?"#b53b2e":"#2879ad";ctx.fillText(`${index+1}. ${point.code} · ${point.y>=0?"+":""}${point.y.toFixed(1)}% · ${formatNumber(point.cases)} kasus`,55,1045+index*24);ctx.fillStyle="#34464a";ctx.fillText(point.description.slice(0,120),390,1045+index*24);});const link=document.createElement("a");link.download=`Scatter_Tarif_iDRG_${new Date().toISOString().slice(0,10)}.png`;link.href=output.toDataURL("image/png");link.click();});
  }
