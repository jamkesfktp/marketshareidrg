...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function renderTariffScatterWorkspace() {
    ...
    const points = [...aggregate.values()].map((item) => { const ina = item.inaTotal / item.cases, idrg = item.idrgTotal / item.cases, delta = (item.idrgTotal - item.inaTotal) / item.inaTotal * 100; return { x: item.cases, y: delta, code: item.code, description: item.description, ina, idrg, cases: item.cases, services: [...item.services].join(", "), segment: delta < 0 ? "negative" : delta <= ui.threshold ? "within" : "high" }; }).filter((item) => Number.isFinite(item.y)).sort((a,b) => Math.abs(b.y) - Math.abs(a.y));
