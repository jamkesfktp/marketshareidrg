...
document.getElementById("globalSimulationSlide").innerHTML = `
  ...
  function hospitalRegional(province) {
    ...
    if (/BANTEN|DKI|JAWA|YOGYAKARTA/.test(p)) return "Reg 1";
    if (/SUMATERA BARAT|RIAU|SUMATERA SELATAN|LAMPUNG|BALI|NUSA TENGGARA BARAT/.test(p)) return "Reg 2";
    if (/ACEH|SUMATERA UTARA|JAMBI|BENGKULU|BANGKA|KEPULAUAN RIAU|KALIMANTAN BARAT|KALIMANTAN TENGAH|KALIMANTAN SELATAN|KALIMANTAN TIMUR|SULAWESI|GORONTALO/.test(p)) return "Reg 3";
    if (/KALIMANTAN UTARA|NUSA TENGGARA TIMUR|MALUKU|PAPUA$|PAPUA BARAT$/.test(p)) return "Reg 4";
